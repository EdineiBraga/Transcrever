...
def transcribe(audio_path):
    ...
    if duration < NO_CHUNK_THRESHOLD:
        ...
        add_usage("transcricao", duration_sec=duration)
    ...
    add_usage("transcricao", duration_sec=total_duration)
...
def process_text(text):
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "REGRAS:\n"
                    "- SEMPRE traduza, mesmo que o texto já esteja em português.\n"
                    "- NÃO mantenha partes no idioma original.\n"
                    "- O resultado final deve estar 100% em português.\n\n"
                    "FORMATO DE SAÍDA:\n"
                    "TRADUÇÃO:\n"
    ...
    add_usage("processamento", input_tokens, estimate_tokens(output))
...
def process_uploaded_file(uploaded_file):
    ...
    audio_path, _ = extract_audio_if_needed(temp_path)
    ...
    with open(tokens_path, "w", encoding="utf-8") as f:
        for k, v in TOKEN_LOG.items():
            f.write(f"{k}: tokens={v['tokens']} cost=${v['cost']:.6f}\n")

    return {
        "usage": TOKEN_LOG,
        "duration": duration,
        "estimated_cost": estimate_transcription_cost(duration),
        "paths": [result_path, tokens_path]
    }